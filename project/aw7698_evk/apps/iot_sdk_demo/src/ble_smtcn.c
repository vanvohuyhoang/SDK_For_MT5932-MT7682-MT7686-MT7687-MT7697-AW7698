...
in project/mt7697_hdk/apps/iot_sdk_demo/GCC/feature.mk.
...
#define BLE_SMTCN_PASSWD_LEN          64 
...
#define BLE_SMTCN_HEADER_LEN        (3 + 2) //3bytes for gatt - 1byte(tagid) - 1byte(tag_length)
...
typedef struct {
    ...
    uint8_t ssidrx_len;
    uint8_t pwrx[BLE_SMTCN_PASSWD_LEN];
    uint8_t pwrx_len;
...
static uint16_t ble_smtcn_get_max_data_len(bt_handle_t connection_handle)
{
    uint16_t mtu = (uint16_t)bt_gattc_get_mtu(connection_handle);
    return mtu - BLE_SMTCN_HEADER_LEN;
}

void ble_smtcn_send_indication(ble_smtcn_wifi_info_id_t tag_id, uint8_t len, uint8_t *value)
{
    /* Should be change to dynamic buffer to save stack. */
    uint8_t buf[64] = {0};
    uint8_t pak[32] = {0}; 
    uint8_t pak_len;
    bt_gattc_charc_value_notification_indication_t *req;
    bt_status_t send_status;
    uint16_t mtu_len = ble_smtcn_get_max_data_len(p_smtcn->conn_handle);

    LOG_I(BLE_SMTCN, "[BLE_SMTCN]ble_smtcn_send_indication, tag_id = %d, mtu = %d\n", tag_id, mtu_len);
    ...
    if (tag_id == BLE_SMTCN_WIFI_INFO_SSID) {
        if (len >= BLE_SMTCN_SSID_LEN && mtu_len >= BLE_SMTCN_SSID_LEN) {
            len = BLE_SMTCN_SSID_LEN;
        } else if (len >= BLE_SMTCN_SSID_LEN && mtu_len < BLE_SMTCN_SSID_LEN) {
            len = mtu_len;
        }
    } else if (tag_id == BLE_SMTCN_WIFI_INFO_IP) {
        if (len >= BLE_SMTCN_SSID_LEN && mtu_len >= BLE_SMTCN_IP_LEN) {
            len = BLE_SMTCN_SSID_LEN;
        } else if (len >= BLE_SMTCN_SSID_LEN && mtu_len < BLE_SMTCN_IP_LEN) {
            len = mtu_len;
        }
    }

    if (tag_id == BLE_SMTCN_WIFI_INFO_SSID
        || tag_id == BLE_SMTCN_WIFI_INFO_IP) {
        pak[1] = len;
        memcpy(pak + 2, value, len);
        pak_len += len + 1;
    }
...
void ble_smtcn_parse_data(uint8_t *data, uint16_t size) 
{
    ...
    LOG_I(BLE_SMTCN, "[BLE_SMTCN]ble_smtcn_parse_data, mtu %d\n", mtu_len);
    
    tag_id = (ble_smtcn_wifi_info_id_t)data[0];
    tag_len = data[1];

    tag_len = ((tag_len > mtu_len)? mtu_len: tag_len);

    LOG_I(BLE_SMTCN, "[BLE_SMTCN]ble_smtcn_parse_data, tag_id = %d, tag_len = %d\n", tag_id, tag_len);
    switch (tag_id) {
        case BLE_SMTCN_WIFI_INFO_SSID:
        {
            if (tag_len == 0 || tag_len > BLE_SMTCN_SSID_LEN) {
                LOG_E(BLE_SMTCN, "[BLE_SMTCN] ssid len error !");
                break;
            }
        ...
        {
            if (tag_len > BLE_SMTCN_PASSWD_LEN) {
                LOG_I(BLE_SMTCN, "[BLE_SMTCN]  passwd error !");
                break;
            }
