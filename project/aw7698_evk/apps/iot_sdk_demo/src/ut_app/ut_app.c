...
uint32_t ble_throughput_task_delay = 200; //ms
#endif
...
bool g_bt_hci_log_enable = false;
...
bt_status_t bt_app_io_callback(void *input, void *output)
{
    ...
    if (UT_APP_CMP("?")) {
        ...
        nvdm_write_data_item("BT_DUO", "HCI_LOG", NVDM_DATA_ITEM_TYPE_RAW_DATA, (const uint8_t *)&g_bt_hci_log_enable, nvdm_size);
    }
...
#ifdef BLE_THROUGHPUT
      bt_gatts_set_max_mtu(258); /* This value should consider with MM Tx/Rx buffer size. */
#else
...
#endif
#ifdef BLE_THROUGHPUT
    else if (UT_APP_CMP("throughput delay")){
        ble_throughput_task_delay = (uint32_t)atoi(cmd + 17);        
        printf("throughput task delay (%d)ms\n", ble_throughput_task_delay);
    }    
#endif
    else {
        int i;
