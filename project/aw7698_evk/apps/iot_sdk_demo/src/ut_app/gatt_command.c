...
extern uint32_t ble_throughput_task_delay;
extern uint32_t ble_tp_task_semaphore;
...
void ble_gatt_send_data()
{
    ...
    do {
        ...
        if (status != BT_STATUS_SUCCESS) {
            uint8_t msg = 1;
            vTaskDelay(ble_throughput_task_delay * portTICK_PERIOD_MS);
            //printf("index %d fail:%x\n", test_index, status);            
            xSemaphoreGive((SemaphoreHandle_t)ble_tp_task_semaphore);
