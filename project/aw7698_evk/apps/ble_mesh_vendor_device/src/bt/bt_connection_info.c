...
app_bt_bonded_info_t *get_bonded_info(const bt_addr_t *target_bt, uint8_t create)
{
    ...
    if (create) {
        ...
        /*mesh gatt provision and proxy service is not requried to support bonding, chapter 7.1.3.1.1/7.1.3.2.1 and chapter 7.2.1.5 in mesh spec*/
        memcpy(&(bonded_info_req[0].bt_addr), target_bt, sizeof(default_bt_addr));
        return &(bonded_info_req[0]);
    }
    //Out of memory
    return NULL;
}
