...
log_create_module(http_client_example, PRINT_LEVEL_INFO);

extern HTTPCLIENT_RESULT httpclient_test_get(void);
extern HTTPCLIENT_RESULT httpclient_test_keepalive(void);
extern void httpclient_test_keepalive_timeout_handle(TimerHandle_t tmr);
extern HTTPCLIENT_RESULT httpclient_test_retrieve(void);
/**
  * @brief      Create a task for http client get example
  * @param[in]  void *args: Not used
  * @return     None
  */
extern int g_test_keepalive_result;
static void user_entry(void *args)
{
    HTTPCLIENT_RESULT ret1 = HTTPCLIENT_ERROR;
    HTTPCLIENT_RESULT ret2 = HTTPCLIENT_ERROR;
    HTTPCLIENT_RESULT ret3 = HTTPCLIENT_ERROR;

	lwip_net_ready();

    // Httpclient "get" feature test
    ret1 = httpclient_test_get();

    // Httpclient "retrieve" feature test
    ret2 = httpclient_test_retrieve();

    // Httpclient "keepalive" feature test
    ret3 = httpclient_test_keepalive();
    if (ret3 == HTTPCLIENT_OK) {
        int count = 0;
        while (g_test_keepalive_result == -1 && count < 60) {
            vTaskDelay(1000 / portTICK_RATE_MS);
            count++;
        }
    }
    LOG_I(http_client_example, "example project ret1=%d ret2=%d ret3=%d g_test_keepalive_result=%d",
            ret1, ret2, ret3, g_test_keepalive_result);

    if (ret1 >= HTTPCLIENT_OK && ret2 >= HTTPCLIENT_OK
            && ret3 >= HTTPCLIENT_OK && g_test_keepalive_result == 1) {
        LOG_I(http_client_example, "example project test success.");
    }
