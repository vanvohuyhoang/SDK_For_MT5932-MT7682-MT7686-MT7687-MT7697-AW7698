...
static void ip_ready_callback(struct netif *netif)
{
    /**
     * Airoha Change
     * Add IPV6 support which the ip_addr is combined with ipv6 and ipv4
     * */
    ip4_addr_t *ip4_addr = ip_2_ip4(&(netif->ip_addr));
    if (!ip4_addr_isany_val(*ip4_addr)) {
    /** Airoha Change End */
        char ip_addr[17] = {0};
        ...
        /**
         * Airoha Change
         * When IPv6 enabled, only give semaphore when IPv6 address is ready
         * */
#if !LWIP_IPV6
        ...
        if (WIFI_MODE_REPEATER == op_mode) {
            ...
            netif_set_addr(ap_if, &netif->ip_addr.u_addr.ip4, &netif->netmask.u_addr.ip4, &netif->gw.u_addr.ip4);
            #elif LWIP_IPV4
            netif_set_addr(ap_if, (const ip4_addr_t *)&netif->ip_addr.addr, (const ip4_addr_t *)&netif->netmask.addr, (const ip4_addr_t *)&netif->gw.addr);
            #endif
        }
#endif
    }
    ...
    for (index = 0; index < LWIP_IPV6_NUM_ADDRESSES; index ++) {
        ip6_addr_t *ip6_addr = (ip6_addr_t *)netif_ip6_addr(netif, index);
        if (ip6_addr_isvalid(netif_ip6_addr_state(netif, index))) {
            if (ip6_addr_islinklocal(ip6_addr)) {
                LOG_I(common, "Link local IPv6 Address : %s", ip6addr_ntoa(ip6_addr));
            } else if (ip6_addr_isglobal(ip6_addr)) {
                LOG_I(common, "Global IPv6 address : %s", ip6addr_ntoa(ip6_addr));
            } else if (ip6_addr_isuniquelocal(ip6_addr)) {
                LOG_I(common, "Unique local IPv6 address : %s", ip6addr_ntoa(ip6_addr));
            } else if (ip6_addr_issitelocal(ip6_addr)) {
                LOG_I(common, "Site local IPv6 address : %s", ip6addr_ntoa(ip6_addr));
            } else {
                LOG_I(common, "Unkonwn IPv6 address : %s", ip6addr_ntoa(ip6_addr));
            }
        }
    }
    LOG_I(common, "**********************************");
    ...
    if ((WIFI_MODE_AP_ONLY != opmode) && WIFI_EVENT_IOT_DISCONNECTED == event) {
        ...
        if (link_status == 0) {
            ...
            if (dhcp_config_init() == STA_IP_MODE_DHCP) {
                #if LWIP_IPV4 && LWIP_IPV6
                netif_set_addr(sta_if, &(IP4_ADDR_ANY->u_addr.ip4), &(IP4_ADDR_ANY->u_addr.ip4), &(IP4_ADDR_ANY->u_addr.ip4));
                #elif LWIP_IPV4
                netif_set_addr(sta_if, (const ip4_addr_t *)&(IP4_ADDR_ANY->addr), (const ip4_addr_t *)&(IP4_ADDR_ANY->addr), (const ip4_addr_t *)&(IP4_ADDR_ANY->addr));
                #endif
            }
...
void lwip_network_init(uint8_t opmode)
{
    ...
    /**
     * Airoha Change
     * Print the current LWIP version
     * */
    #include "init.h"
    LOG_I(common, "==================================================");
    LOG_I(common, "Current LWIP Version : %d.%d.%d", LWIP_VERSION_MAJOR, LWIP_VERSION_MINOR, LWIP_VERSION_REVISION);
    LOG_I(common, "==================================================");
    /** Airoha Change End */
