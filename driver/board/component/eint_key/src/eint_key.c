...
void bsp_eint_key_timer_callback(TimerHandle_t xTimer)
{
    ...
    if (i>=4) {
        log_hal_msgid_info("[eint_key]no timer found, xTimer=%d\r\n",1,xTimer);
        return;
    }
    ...
    {
        log_hal_msgid_info("[eint_key]repeat_time xTimerChangePeriodFromISR fail, index = %d\r\n",1,i);
    }
...
void bsp_eint_key_callback(uint8_t *key_number)
{
    ...
    if (eint_key_context.is_init_ready != true) {
        ...
        {
            log_hal_msgid_info("[eint_key]xTimerStopFromISR fail, index = %d is_init_ready\r\n",1,index);
        }
    ...
    if (eint_key_context.key_sate[index] == BSP_EINT_KEY_RELEASE) {
        ...
        {
            log_hal_msgid_info("[eint_key]longpress_time xTimerChangePeriodFromISR fail, index = %d\r\n",1,index);
        }
        
        if( xTimerStartFromISR( eint_key_context.rtos_timer[index], &xHigherPriorityTaskWoken ) != pdPASS )
        {
            log_hal_msgid_info("[eint_key]xTimerStartFromISR fail, index = %d\r\n",1,index);
        }
    }
    else {
        eint_key_context.key_sate[index] = BSP_EINT_KEY_RELEASE;
        ...
        if( xTimerStopFromISR( eint_key_context.rtos_timer[index], &xHigherPriorityTaskWoken ) != pdPASS )
        {
            log_hal_msgid_info("[eint_key]xTimerStopFromISR fail, index = %d\r\n",1,index);
        }
    } 
...
void bsp_eint_key_timer_callback(uint8_t *key_number)
{
    ...
    if (ret != HAL_GPT_STATUS_OK) {
        log_hal_msgid_info("[eint_key]longpress_time start repeat timer fail, index = %d, ret=%d\r\n",2,index,ret);
    }
...
void bsp_eint_key_callback(uint8_t *key_number)
{
    ...
    if (eint_key_context.key_sate[index] == BSP_EINT_KEY_RELEASE) {
        ...
        if (ret != HAL_GPT_STATUS_OK) {
            log_hal_msgid_info("[eint_key]longpress_time get timer fail, index = %d, ret=%d\r\n",2,index,ret);
        }
        ...
        ret = hal_gpt_sw_start_timer_ms(eint_key_context.timer_handle[index], \
                                        ...
                                        (void*)(&eint_key_context.index[index]));
        if (ret != HAL_GPT_STATUS_OK) {
            log_hal_msgid_info("[eint_key]longpress_time start timer fail, index = %d, ret=%d\r\n",2,index,ret);
        }
        ...
        if (ret != HAL_GPT_STATUS_OK) {
            log_hal_msgid_info("[eint_key]longpress_time stop timer fail, index = %d, ret=%d\r\n",2,index,ret);
        }

        ret = hal_gpt_sw_free_timer(eint_key_context.timer_handle[index]);
        if (ret != HAL_GPT_STATUS_OK) {
            log_hal_msgid_info("[eint_key]longpress_time free timer fail, index = %d, ret=%d\r\n",2,index,ret);
        }
    } 
...
bool bsp_eint_key_init(bsp_eint_key_config_t *config)
{
    ...
    if (config == NULL ) {
        log_hal_msgid_info("[eint_key]config pioter is null\r\n", 0);
        return false;
    }

    if (eint_key_context.has_initiliazed == true) {
        log_hal_msgid_info("[eint_key]bsp eint key has initiliazed\r\n", 0);
        return false;
    }       
    ...
    for(i = 0; i<BSP_EINT_KEY_NUMBER; i++){
        ...
        log_hal_msgid_info("[eint_key][key%d]GPIO    = %d",2, i,eint_key_mapping[i].gpio_port);
        log_hal_msgid_info("[eint_key][key%d]eint    = %d",2, i,eint_key_mapping[i].eint_number);
        log_hal_msgid_info("[eint_key][key%d]key     = %d",2, i,eint_key_mapping[i].key_data);
        #if 0
        log_hal_msgid_info("[eint_key]eint mask status = %x\r\n",1, *(volatile uint32_t*)0xa2030320);
        log_hal_msgid_info("[eint_key]eint ack  status = %x\r\n",1, *(volatile uint32_t*)0xa2030300);
        #endif

    }

    #if 0
    log_hal_msgid_info("[eint_key]key valid num  = %d",1, count);
    log_hal_msgid_info("[eint_key]longpress time = %d ms",1, eint_key_context.longpress_time);
    log_hal_msgid_info("[eint_key]repeat    time = %d ms",1, eint_key_context.repeat_time);
    #endif
...
{
    if (callback == NULL) {
        log_hal_msgid_info("[eint_key]callback pioter is null\r\n", 0);
        return false;
    }
