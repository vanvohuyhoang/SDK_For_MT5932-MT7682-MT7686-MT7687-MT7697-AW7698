...
uint8_t g_wfc_current_dbg_level = WFC_DBG_LEVEL_ERROR;
...
void wfc_memq_handler(void)
{
    ...
    WFC_DEBUG_INFO("Enter memq handler.\r\n");
    ...
    wfc_if_init();
    WFC_DEBUG_INFO("wfc interface init done.\r\n");
...
uint8_t wfc_send_queue(uint16_t op, uint16_t cmd, uint8_t *ptr, uint32_t ptrSize, uint8_t fromISR)
{
    ...
    WFC_DEBUG_INFO("Send wfc_queue,op:%d,cmd:%d.\r\n",op,cmd);
    if (fromISR)
    {
...
uint8_t wfc_send_to_host(uint8_t *pbuf, uint32_t bufSize, uint8_t isEvt)
{
    ...
    if (isEvt)
    {
        WFC_DEBUG_INFO("Copy evt buffer.\r\n");
#ifdef  MTK_WIFI_STUB_CONF_SPI_ENABLE
...
void wfc_stub_handler(T_WFC_OPCODE *pwfc)
{
    ...
    //WFC_DEBUG_INFO("OpCode(%d), OpCmd(%d)\r\n", pwfc->opcode, pwfc->opcmd);
    ...
    switch(pwfc->opcmd)
    {
        case WFC_CMD_DTH_READY: //Host Receive Buffer Ready
        {
            ...
            if ( g_hrx_qbuf == NULL )
            {
      	        WFC_DEBUG_ERROR("[HRX]: Error - No HRX Data to Send\r\n");
        ...
        {
            wfc_rx_notify();
            WFC_DEBUG_INFO("Give wake up response.\r\n");
        ...
        {
            WFC_DEBUG_INFO("<WFC> - g_ip_ready(%d)\r\n",g_ip_ready);
