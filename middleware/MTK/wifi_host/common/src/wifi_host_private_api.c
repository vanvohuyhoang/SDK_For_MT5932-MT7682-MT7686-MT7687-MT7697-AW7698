...
uint32_t wifi_host_set_debug_level(uint8_t debug_level)
{
    ...
    int32_t ret;    
    wh_set_debug_log_cmd_para_t cmd_para;
    wh_set_debug_log_cmd_resp_para_t *rsp_para;
    
    cmd_para.api_id.api = WFC_API_DEBUG_LEVEL;
    cmd_para.debug_level = debug_level;
    cmd_para.api_id.set = true;
    wfcm_to_slave(WFC_CMD_API, (uint8_t *)&cmd_para, sizeof(cmd_para), (uint8_t *)&rsp_buf, &rsp_size, 0);
    rsp_para = (wh_set_debug_log_cmd_resp_para_t *)rsp_buf;
    ret = rsp_para->error_code;
